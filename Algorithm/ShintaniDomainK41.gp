...
tim1=getwalltime();
 ...
 if(r>1,
   ...
   C3=vector(r-1,j,kmultcomplex(ut[j],C1,p)); Qr=C3[r-1];
   C3=concat(C3);
...
ShintaniExamplesK41(L)= \\ return a file of explicit Shintani domains from a list as input L=[list of polynomials]
{my(F,tm);
write(ShintaniK41,"examples = [\\");
for(j=1,#L-1,
   tm=getwalltime();
   F=FDK41(L[j]);
   write(ShintaniK41,"",F,",\\");
...
kdiffCones(P,Q,bnf)= \\ given two pointed cones P and Q, where Q=P\cap P' is intersection of two cones given as: [genrs, ineqs, +/-1]
...
kcomplex(S,bnf)=  \\ Here attractor S:=Attractor(bnf,d) obtained below is union of "d" full-dim. k-rational cones with "2d" vertices
{my(P,C,q,h,a);
...
/*...End of the algorithm FDK41(p) to obtain a Shintani domain given p=quartic polynomial which defines a totally complex number field*/
